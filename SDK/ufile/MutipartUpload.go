...
func (u *Request) AsyncUpload(filePath, keyName, mimeType string, jobs int) error {
	if jobs <= 0 {
		jobs = 1
	}
...
func (u *Request) InitiateMultipartUpload(keyName, mimeType string) (*MultipartState, error) {
...
func (u *Request) UploadPart(buf *bytes.Buffer, state *MultipartState, partNumber int) error {
	query := &url.Values{}
	...
	resp, err := u.requestWithResponse(req)
	if err != nil {
		return err
	}
...
func (u *Request) AbortMultipartUpload(state *MultipartState) error {
	query := &url.Values{}
	query.Add("uploadId", state.uploadID)
	reqURL := u.genFileURL(state.keyName) + "?" + query.Encode()
...
//state 参数是 InitiateMultipartUpload 返回的
func (u *Request) FinishMultipartUpload(state *MultipartState) error {
	query := &url.Values{}
	query.Add("uploadId", state.uploadID)
	reqURL := u.genFileURL(state.keyName) + "?" + query.Encode()
	var etagsStr string
